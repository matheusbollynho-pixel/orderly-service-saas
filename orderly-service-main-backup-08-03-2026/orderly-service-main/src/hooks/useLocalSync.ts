...
export function useLocalSync() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const handleLocalUpdate = (event: CustomEvent<{ type: string; data: unknown }>) => {
        const { type, data } = event.detail;
        console.log('📡 Atualização local recebida:', type, data);

        // Atualizar queries relevantes imediatamente
  ...
  const notifyUpdate = useCallback((type: string, data?: unknown) => {
