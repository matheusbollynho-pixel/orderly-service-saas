...
export function useMechanics() {
  ...
  // Diagnóstico: log de erro da query
  if (mechanicsQuery.error) {
    console.error('Erro mechanicsQuery:', mechanicsQuery.error);
  }
  ...
  const createMechanic = useMutation({
    ...
    onError: (e: Error | unknown) => {
      toast.error(`Erro ao cadastrar mecânico: ${(e as Error)?.message || 'Erro desconhecido'}`);
    },
  });
