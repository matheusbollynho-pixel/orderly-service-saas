...
export function MaintenanceDebugPanel() {
  ...
  const [autoTests, setAutoTests] = useState<Record<string, unknown>>({});
    ...
    try {
      ...
      try {
        ...
        newResults[1] = {
          ...
          details: keywords?.map((k: { keyword: string; reminder_days: number }) => `${k.keyword} (${k.reminder_days} dias)`).join(', '),
        };
      } catch (e) {
      ...
      try {
        ...
        const pending = reminders?.filter((r: { reminder_sent_at: string | null }) => !r.reminder_sent_at).length || 0;
      ...
      try {
        ...
        reminders?.forEach((r: { maintenance_keywords?: { keyword?: string } }) => {
