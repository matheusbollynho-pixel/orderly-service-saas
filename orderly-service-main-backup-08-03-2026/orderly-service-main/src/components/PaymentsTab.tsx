...
export function PaymentsTab({ orders, isLoading, period, onPeriodChange, onAddPayment, onDeletePayment, onUpdatePayment }: PaymentsTabProps) {
  ...
  const methodTotals = useMemo(() => {
    ...
    filteredOrders.forEach((o: ServiceOrder & {
      _paymentsInRange?: Array<{
        id: string;
        created_at: string;
        amount?: number;
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
        <Input
          ...
          className="h-10"
        />
        <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as 'all' | 'pending' | 'completed_unpaid')}>
        ...
        <Select value={period} onValueChange={(v) => onPeriodChange(v as 'week' | 'month' | 'all')}>
          <SelectTrigger className="w-full h-10">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {filteredOrders.map((o: ServiceOrder & {
            _paymentsInRange?: Array<{
              id: string;
              created_at: string;
              amount?: number;
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    {o._paymentsInRange && o._paymentsInRange.length > 0 && (
                      <p className="text-xs text-muted-foreground">Último pgto: {new Date(Math.max(...o._paymentsInRange.map((p) => new Date(p.created_at).getTime()))).toLocaleDateString('pt-BR')}</p>
                    )}
