...
export function ExpressCadastroPage({ onBack, onOrderCreated }: ExpressCadastroPageProps) {
  ...
  // Campo de seleção de atendente
  const renderAtendenteSelect = () => (
    <div className="mb-4">
      <Label htmlFor="atendente">Responsável pelo atendimento *</Label>
      <Select value={atendimentoId} onValueChange={setAtendimentoId} required>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!atendimentoId.trim()) {
      toast.error('Selecione o responsável pelo atendimento antes de criar a OS Express.');
      return;
    }
    if (!client.name.trim()) {
              ...
              <div className="grid grid-cols-1 gap-2">
                {searchResults.map((c) => (
                  <button
                    ...
                    className="text-left px-3 py-2 rounded-lg border border-border/50 hover:bg-muted transition"
                  >
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder={VEHICLE_CAP === 'Carro' ? 'Uno 1.0' : 'CG 160'}
              spellCheck
              lang="pt-BR"
              className="h-11"
            />
