...
export function PrintOrderPage() {
  ...
  const autoActionRef = useRef<{ executed: boolean; action: string | null }>({ executed: false, action: null })
    ...
    const loadOrder = async () => {
      ...
      try {
        ...
        if (!looksLikeUuid) {
          ...
          const found = (recentOrders || []).find((o: Record<string, unknown>) =>
        ...
        const formatCurrency = (value: string | number) => parseFloat(String(value)) || 0
        ...
        const checklistRaw = (order?.checklist_items || []) as Record<string, unknown>[]

        const observacaoChecklistItem = checklistRaw.find((item: Record<string, unknown>) =>
          normalizeLabel(item?.label as string || "").includes("OBSERVAC")
        )
        ...
        const checklistItems: ChecklistItem[] = checklistRaw
          .filter((item: Record<string, unknown>) => !normalizeLabel(item?.label as string || "").includes("OBSERVAC"))
        ...
        const serviceItems: ServiceItem[] = (order?.materials || []).map((item: Record<string, unknown>) => {
        ...
        const descontoCalc = (order?.payments || []).reduce(
          (acc: number, p: Record<string, unknown>) => acc + formatCurrency(p?.discount_amount),
          0
        )
        const totalPagoCalc = (order?.payments || []).reduce(
          (acc: number, p: Record<string, unknown>) => acc + formatCurrency(p?.amount),
          0
        )
        ...
        const motorcycle = (order as Record<string, unknown>)?.motorcycles
        ...
        const completedAt = (order as Record<string, unknown>)?.completed_at ||
        ...
        setNotaData({
          ...
          observacaoPagamento: (order?.payments || []).map((p: Record<string, unknown>) => p?.method).filter(Boolean).join(", "),
          status: statusMap[order?.status] || order?.status || "PENDENTE",
          signatureChecklist: (order as Record<string, unknown>)?.signature_data || null,
          signatureCliente: (order as Record<string, unknown>)?.delivery_signature_data || null,
        })
  ...
  const handleSendWhatsApp = useCallback(async () => {
    ...
    try {
      ...
      alert((error as Error)?.message || 'Erro ao enviar PDF. Tente novamente.')
    } finally {
  ...
  // Detectar se veio de redirect e auto-executar ação (apenas uma vez)
  useEffect(() => {
    // Só executar se os dados estão carregados e ainda não foi executado
    if (loading || !notaData || autoActionRef.current.executed) {
      return
    }

    // Verificar se há uma ação de auto-envio
    if (location.state?.sendWhatsApp) {
      console.log('[PrintOrderPage] Auto-triggering WhatsApp send');
      autoActionRef.current.executed = true
      autoActionRef.current.action = 'whatsapp'
      
      setTimeout(() => {
        handleSendWhatsApp()
      }, 500)
    } else if (location.state?.autoDownload) {
      console.log('[PrintOrderPage] Auto-triggering PDF download');
      autoActionRef.current.executed = true
      autoActionRef.current.action = 'download'
      
      setTimeout(() => {
        handleDownloadPDF()
      }, 500)
    }
  }, [loading, notaData, location.state?.sendWhatsApp, location.state?.autoDownload, handleSendWhatsApp, handleDownloadPDF])
